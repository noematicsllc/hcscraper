#!/usr/bin/env python3
"""Test end-to-end flow: authenticate and download a single billing document.

This script tests the complete flow from authentication to billing document extraction.

Usage:
    python test_single_billing_document.py <billing_document_id>

Example:
    python test_single_billing_document.py 5062633071
"""
...
def main():
    """Test end-to-end billing document extraction."""
    # Check for billing document ID argument
    if len(sys.argv) < 2:
        print("Usage: python test_single_billing_document.py <billing_document_id>")
        print("\nExample:")
        print("  python test_single_billing_document.py 5062633071")
        return 1

    billing_document_id = sys.argv[1]
    ...
    print("=" * 70)
    print("Hallmark Connect - Single Billing Document Extraction Test")
    print("=" * 70)
    print(f"\nBilling Document ID: {billing_document_id}")
    ...
    # Step 3: Extract billing document
    print(f"\n[3/3] Extracting billing document {billing_document_id}...")
    print("-" * 70)

    try:
        extractor = BillingDocumentExtractor(
        ...
        success = extractor.extract_single_billing_document(billing_document_id)

        if success:
            print("\n" + "=" * 70)
            print("✓ Billing document extraction successful!")
            print("=" * 70)
            print(f"\nFiles saved to: {config.output_directory}/")
            print(f"  - billing_{billing_document_id}_*.json")
            print(f"  - billing_{billing_document_id}_*.csv")
            return 0
        else:
            print("\n✗ Billing document extraction failed")
            return 1
