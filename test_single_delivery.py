#!/usr/bin/env python3
"""Test end-to-end flow: authenticate and download a single delivery.

This script tests the complete flow from authentication to delivery extraction.

Usage:
    python test_single_delivery.py <delivery_id>

Example:
    python test_single_delivery.py 8412345678
"""
...
from src.extractors.delivery_extractor import DeliveryExtractor
from src.utils.config import get_config
from src.utils.logger import setup_logging


def main():
    """Test end-to-end delivery extraction."""
    # Check for delivery ID argument
    if len(sys.argv) < 2:
        print("Usage: python test_single_delivery.py <delivery_id>")
        print("\nExample:")
        print("  python test_single_delivery.py 8412345678")
        return 1
    ...
    print("=" * 70)
    print("Hallmark Connect - Single Delivery Extraction Test")
    print("=" * 70)
    ...
    # Step 3: Extract delivery
    print(f"\n[3/3] Extracting delivery {delivery_id}...")
    print("-" * 70)

    try:
        extractor = DeliveryExtractor(
        ...
        success = extractor.extract_single_delivery(delivery_id)

        if success:
            print("\n" + "=" * 70)
            print("✓ Delivery extraction successful!")
            print("=" * 70)
            print(f"\nFiles saved to: {config.output_directory}/")
            print(f"  - delivery_{delivery_id}_*.json")
            print(f"  - delivery_{delivery_id}_*.csv")
            return 0
        else:
